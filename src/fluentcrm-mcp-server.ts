...
class FluentCRMClient {
    ...
    this.blockliApiClient.interceptors.response.use(
      ...
      error => {
        ...
        const detail = data && Object.keys(data).length > 0
          ? ` Full response: ${JSON.stringify(data)}`
          : '';
        const err: any = new Error(`Blockli API Error: ${message}${detail}`);
        err.httpStatus   = error.response?.status;
        err.responseData = data;
        throw err;
      }
  ...
  async getBlockliCampaign(id: number) {
      ...
      },
      {
        ...
        inputSchema: {
          ...
          properties: {
            ...
            subjects:        { type: 'array',   items: { type: 'string' }, description: t('fluentcrm_create_campaign', 'subjects') },
            ...
            advanced_filters: {
              ...
              description: t('fluentcrm_create_campaign', 'advanced_filters'),
            },
          },
  ...
  try {
    switch (name) {
      ...
      case 'fluentcrm_get_campaign_detail':
        return { content: [{ type: 'text', text: JSON.stringify(await client.getBlockliCampaign((args as any)?.id), null, 2) }] };
